...
public class OrderPaymentSucceededTests : StateMachineTestFixture<OrderProccesingStateMachine, OrderState>
{
    ...
    public async Task Should_publish_OrderStatusChangedToAwaitingValidation()
    {
        ...
        var orderPaymentSucceeded = new OrderPaymentSucceeded(orderStarted.OrderId);
        ...
        Assert.IsTrue(await TestHarness.Published.Any<OrderStatusChangedToPaid>(), "Message not published");

        await AdvanceSystemTime(TimeSpan.FromSeconds(2));

        var instance = SagaHarness.Created.ContainsInState(orderId, Machine, Machine.Paid);
