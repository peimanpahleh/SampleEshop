...
public class OrderPaymentFailedActivity : Activity<OrderState>
{
    private readonly ILogger<OrderPaymentFailedActivity> _logger;

    public OrderPaymentFailedActivity(ILogger<OrderPaymentFailedActivity> logger)
    {
        _logger = logger;
    }
    ...
    {
        var scopeName = "OrderPaymentFailedActivity".ToLower();
    ...
    private async Task Execute(BehaviorContext<OrderState> context)
    {
        ...
        _logger.LogInformation("OrderPaymentFailedActivity published OrderStatusChangedToCancelled");
    }
}
